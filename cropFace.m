...
dist = round(abs(eyeCenter - mouthPos)); % find distance between mouth and center of eyes

xmin = max((eyeCenter(1) - eyedist(1)),0); % min X value
ymin = max((eyeCenter(2)-(3/5 * dist(2))), 0); % min Y value, 1*dist(2) cuts of bit of mouth 

width = 2*eyedist(1); %horizontal distance
height = 2*dist(2); %vertical distance, 2* crops chin and x>2 dont
